...
double ptlepBins[5]={20,40,70,110,400};
...
void test::fill_hist(double fill_ptlep1, vector<double> Bins, TH1D* hist, double weight){
    double content = 0;
...
void test::fill_gen_hist(double fill_ptlep1, double gen_ptlep1, vector<double> Bins, TH2D* hist, double weight){
    double content_reco = 0, content_gen = 0;
    for(int i = 0; i < Bins.size(); i++){
       if(i < (Bins.size()-2)) { 
           if(fill_ptlep1 >= Bins[i] && fill_ptlep1 < Bins[i+1]) {
               content_reco = i + 0.5;
               for(int j = 0; j < Bins.size(); j++){
                  if(j < (Bins.size()-2)) {
                     if(gen_ptlep1 >= Bins[j] && gen_ptlep1 < Bins[j+1]) {content_gen = j + 0.5;}
                  }
                  else if(j == (Bins.size()-2)){
                     if(gen_ptlep1 >= Bins[j] ) {content_gen = j + 0.5;}
                  }
               }
           }
       }
       else if(i == (Bins.size()-2)){
           if(fill_ptlep1 >= Bins[i] ) {
              content_reco = i + 0.5;
               for(int j = 0; j < Bins.size(); j++){
                  if(j < (Bins.size()-2)) {
                     if(gen_ptlep1 >= Bins[j] && gen_ptlep1 < Bins[j+1]) {content_gen = j + 0.5;}
       ...
       //hist->Fill(content_gen, content_reco, weight);
    }
    hist->Fill(content_gen, content_reco, weight);
}
...
void test::creat_hist(){
   //bins.push_back(Bins);
   int i = 0;
   const int BINS_size = Bins.size() - 1;

      
   for(int j = 0; j<21; j++){
      h[j] = new TH1D("hist_"+uncertainty_hist_name[j],"hist_" +uncertainty_hist_name[j], BINS_size, 0, BINS_size); 
      h_in[j] = new TH2D("hist_in_"+uncertainty_hist_name[j],"hist_in_" +uncertainty_hist_name[j], BINS_size, 0, BINS_size, BINS_size, 0, BINS_size);
      h_out[j] = new TH1D("hist_out_"+uncertainty_hist_name[j],"hist_out_" +uncertainty_hist_name[j], BINS_size, 0, BINS_size);

   }
    
   for(int j2 = 0; j2<4; j2++){
      h_jecr[j2] = new TH1D("hist_"+uncertainty_jecr[j2],"hist_"+uncertainty_jecr[j2], BINS_size, 0, BINS_size);   
      h_in_jecr[j2] = new TH2D("hist_in_"+uncertainty_jecr[j2],"hist_in_"+uncertainty_jecr[j2], BINS_size, 0, BINS_size, BINS_size, 0, BINS_size);
      h_out_jecr[j2] = new TH1D("hist_out_"+uncertainty_jecr[j2],"hist_out_"+uncertainty_jecr[j2], BINS_size, 0, BINS_size);
...
//cout<<fill_Mva<<fill_Mva_JEC_up<<endl;
      ...
      if (fabs(fill_photonsceta) < pho_eta_high && fabs(fill_photonsceta) > pho_eta_low){
         ...
         if(m_type == "data") {
            ...
            weight = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;

            weight_L1_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;
            weight_L1_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;

            weight_photon_ID_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_up_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;
            weight_photon_ID_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_low_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;

            weight_electron_ID_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_up_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;
            weight_electron_ID_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_low_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;

            weight_electron_Reco_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_up_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;
            weight_electron_Reco_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_low_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;

            weight_electron_HLT_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_up_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;
            weight_electron_HLT_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_low_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;


            weight_muon_ID_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_up_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;
            weight_muon_ID_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_low_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;

            weight_muon_iso_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_up_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;
            weight_muon_iso_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_low_SF * muon_HLT_SF*btag_jet1_SF * btag_jet2_SF;

            weight_muon_HLT_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_up_SF * btag_jet1_SF * btag_jet2_SF;
            weight_muon_HLT_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_low_SF * btag_jet1_SF * btag_jet2_SF;

            weight_btag_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_up_SF * btag_jet2_up_SF;
            weight_btag_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_low_SF * btag_jet2_low_SF;

            weight_jec_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF_JEC_up * btag_jet2_SF_JEC_up;
            weight_jec_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF_JEC_down * btag_jet2_SF_JEC_down;
            weight_jer_up = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF_JER_up * btag_jet2_SF_JER_up;
            weight_jer_down = (1) * cross_section_SF * pu_weight_SF * photon_ID_SF * electron_ID_SF * electron_Reco_SF * electron_HLT_SF *muon_ID_SF * muon_iso_SF * muon_HLT_SF*btag_jet1_SF_JER_down * btag_jet2_SF_JER_down;
            ...
            if(cut1){ 
//cout<<jentry<<" genjet_pt list:  "<<genjet_pt[0]<<" "<<genjet_pt[1]<<" "<<genjet_pt[2]<<" "<<genjet_pt[3]<<" "<<genjet_pt[4]<<" "<<genjet_pt[5]<<endl;
//cout<<jentry<<" genjet_eta list: "<<genjet_eta[0]<<" "<<genjet_eta[1]<<" "<<genjet_eta[2]<<" "<<genjet_eta[3]<<" "<<genjet_eta[4]<<" "<<genjet_eta[5]<<endl;
                  //cout<<jentry<<" eta: "<<genjet1eta<< " "<<genjet2eta << " "<<fill_jet1eta<<" "<<fill_jet2eta<<endl;
                  //cout<<jentry<<" pt: "<<genjet1pt<< " "<<genjet2pt << " "<<fill_jet1pt<<" "<<fill_jet2pt<<endl;
               for(int j = 0; j<21; j++){
                  fill_hist(fill_ptlep1, Bins, h[j], fill_weight[j]);
//cout<<jentry<<" genjet_pt list:  "<<genjet_pt[0]<<" "<<genjet_pt[1]<<" "<<genjet_pt[2]<<" "<<genjet_pt[3]<<" "<<genjet_pt[4]<<" "<<genjet_pt[5]<<endl;
//cout<<jentry<<" genjet_eta list: "<<genjet_eta[0]<<" "<<genjet_eta[1]<<" "<<genjet_eta[2]<<" "<<genjet_eta[3]<<" "<<genjet_eta[4]<<" "<<genjet_eta[5]<<endl;
//                  cout<<jentry<<" eta: "<<genjet1eta<< " "<<genjet2eta << " "<<fill_jet1eta<<" "<<fill_jet2eta<<endl;
//                  cout<<jentry<<" pt: "<<genjet1pt<< " "<<genjet2pt << " "<<fill_jet1pt<<" "<<fill_jet2pt<<endl;

                 //if (cut_gen) fill_gen_hist(fill_ptlep1, genlep1pt, Bins, h_in[j], fill_weight[j]);
                  //if (!cut_gen) fill_hist(fill_ptlep1, Bins, h_out[j], fill_weight[j]);

               }
            }
            if (cut2) {
               fill_hist(fill_ptlep1, Bins, h_jecr[0], fill_weight_jecr[0]);
               //if (cut_gen) fill_gen_hist(fill_ptlep1, genlep1pt, Bins, h_in_jecr[0], fill_weight_jecr[0]);
               //if (!cut_gen) fill_hist(fill_ptlep1, Bins, h_out_jecr[0], fill_weight_jecr[0]);

            }
            if (cut3) {
               fill_hist(fill_ptlep1, Bins, h_jecr[1], fill_weight_jecr[1]);
               //if (cut_gen) fill_gen_hist(fill_ptlep1, genlep1pt, Bins, h_in_jecr[1], fill_weight_jecr[1]);
               //if (!cut_gen) fill_hist(fill_ptlep1, Bins, h_out_jecr[1], fill_weight_jecr[1]);
            }
            if (cut4) {
               fill_hist(fill_ptlep1, Bins, h_jecr[2], fill_weight_jecr[2]);
               //if (cut_gen) fill_gen_hist(fill_ptlep1, genlep1pt, Bins, h_in_jecr[2], fill_weight_jecr[2]);
               //if (!cut_gen) fill_hist(fill_ptlep1, Bins, h_out_jecr[2], fill_weight_jecr[2]);
            }
            if (cut5) {
               fill_hist(fill_ptlep1, Bins, h_jecr[3], fill_weight_jecr[3]);
               //if (cut_gen) fill_gen_hist(fill_ptlep1, genlep1pt, Bins, h_in_jecr[3], fill_weight_jecr[3]);
               //if (!cut_gen) fill_hist(fill_ptlep1, Bins, h_out_jecr[3], fill_weight_jecr[3]);
            }
