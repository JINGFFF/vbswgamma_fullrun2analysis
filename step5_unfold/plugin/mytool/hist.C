...
int fill_hist(TString region,double fill_Mjj, double fill_deltaeta, TH1D* hist, double weight){
      int bin_num = -1000;
      if(region == "signal"){
         if(fill_Mjj>500   &&fill_Mjj<600   &&fill_deltaeta>=30  &&fill_deltaeta<80)  {bin_num = 0; hist->Fill(0.5,weight);}//0~1, 2.5~4.5 and 500~800
         if(fill_Mjj>=600  &&fill_Mjj<700   &&fill_deltaeta>=30  &&fill_deltaeta<80)  {bin_num = 1; hist->Fill(1.5,weight);}//1~2 2.5~4.5 and 800~1200
         if(fill_Mjj>=700  &&fill_Mjj<1000  &&fill_deltaeta>=30  &&fill_deltaeta<80)  {bin_num = 2; hist->Fill(2.5,weight);}//2~3 2.5~4.5 1200~2000
         if(fill_Mjj>=1000                  &&fill_deltaeta>=30  &&fill_deltaeta<80)  {bin_num = 3; hist->Fill(3.5,weight);}//5~6 6~infi 500~800

         if(fill_Mjj>=500  &&fill_Mjj<600   &&fill_deltaeta>=80  &&fill_deltaeta<130) {bin_num = 4; hist->Fill(4.5,weight);}//3~4 4.5~6 500~800 
         if(fill_Mjj>=600  &&fill_Mjj<700   &&fill_deltaeta>=80  &&fill_deltaeta<130) {bin_num = 5; hist->Fill(5.5,weight);}//4~5 4.5~6 800~1200
         if(fill_Mjj>=700  &&fill_Mjj<1000  &&fill_deltaeta>=80  &&fill_deltaeta<130) {bin_num = 6; hist->Fill(6.5,weight);}//4~5 4.5~6 800~1200
         if(fill_Mjj>=1000                  &&fill_deltaeta>=80  &&fill_deltaeta<130) {bin_num = 7; hist->Fill(7.5,weight);}//5~6 6~infi 500~800

         if(fill_Mjj>=500  &&fill_Mjj<600   &&fill_deltaeta>=130)                     {bin_num = 8; hist->Fill(8.5,weight);}//6~7 6~infi 800~1200
         if(fill_Mjj>=600  &&fill_Mjj<700   &&fill_deltaeta>=130)                     {bin_num = 9; hist->Fill(9.5,weight);}//7~8 6~infi800~1200
         if(fill_Mjj>=700  &&fill_Mjj<1000  &&fill_deltaeta>=130)                     {bin_num = 10; hist->Fill(10.5,weight);}//7~8 6~infi800~1200
         if(fill_Mjj>=1000                  &&fill_deltaeta>=130)                     {bin_num = 11; hist->Fill(11.5,weight);}//8~9 6~infi 800~1200

      }
      if(region == "control"){
         if(fill_Mjj>200&&fill_Mjj<300) {bin_num = 0; hist->Fill(0.5,weight);}//0~1, 2.5~4.5 and 500~800
         if(fill_Mjj>=300&&fill_Mjj<400){bin_num = 1; hist->Fill(1.5,weight);}//1~2 2.5~4.5 and 800~1200
         if(fill_Mjj>=400&&fill_Mjj<500){bin_num = 2; hist->Fill(2.5,weight);}//3~4 4.5~6 500~800 
...
void creat_hist(TString region){
   ...
   for(int j = 0; j<length; j++){
      ...
      for (int ii = 0; ii < h[j]->GetNbinsX(); ii++){
         ...
         ss<<ii;
         TString x = ss.str();
         h_in[ii][j] = new TH1D("hist_in_"+x+"_"+uncertainty_hist_name[j],"hist_in_" +x+"_"+uncertainty_hist_name[j], 12, 0, 12);
      }
   ...
   for(int j2 = 0; j2<4; j2++){
      ...
      for (int ii2 = 0; ii2 < h_jecr[j2]->GetNbinsX(); ii2++){
         ...
         ss<<ii2;
         TString x = ss.str();
         h_in_jecr[ii2][j2] = new TH1D("hist_in_"+x+"_"+uncertainty_jecr[j2],"hist_in_"+x+"_"+uncertainty_jecr[j2], 12, 0, 12);
      }
   ...
   for(int j = 0; j<length; j++){
      ...
      for (int ii = 0; ii < h[j]->GetNbinsX(); ii++){
         ...
         ss<<ii;
         TString x = ss.str();
         h_in[ii][j] = new TH1D("hist_in_"+x+"_"+uncertainty_hist_name[j],"hist_in_" +x+"_"+uncertainty_hist_name[j], 3, 0, 3);
      }
   ...
   for(int j2 = 0; j2<4; j2++){
      ...
      for (int ii2 = 0; ii2 < h_jecr[j2]->GetNbinsX(); ii2++){
         ...
         ss<<ii2;
         TString x = ss.str();
         h_in_jecr[ii2][j2] = new TH1D("hist_in_"+x+"_"+uncertainty_jecr[j2],"hist_in_"+x+"_"+uncertainty_jecr[j2], 3, 0, 3);
      }
