  ...
  // TTreeReaderValue<Double_t>    prefWeightDown  = {fReader, "prefWeightDown"};
   ...
   while (fReader.Next()) {
      ...
      if (jentry % 10000 == 0){ 
         ...
         string s1(ks,'>');
         string s2(50-ks,'-');
         cout<<n<<"  processing : ["<<"\033[32m"<<s1<<"\033[0m"<<s2<<"]  "<<"\033[33m"<<100.*jentry/maxEntries<<"%  "<<"p_event: "<<p_event<<" n_event: "<<n_event<<"\033[0m"<<endl;
         }
//cout<<jentry<<endl;
      jentry++;
...
//cout<<
   ...
   bool cut_test1, cut_test2, cut_test3;
   if(m_channel == "muon") {
      cut_test1 = fill_HLT_muon==1 && abs(fill_lep)==13 && fill_ptlep1>30. && fabs(fill_etalep1)<2.4
             && fill_ngoodmus==1 && fill_ngoodeles==0 &&(fill_nloosemus+fill_nlooseeles)==1;
   }
   if(m_channel == "electron") {
      cut_test1 = fill_HLT_electron==1 && abs(fill_lep)==11 && fill_ptlep1>30. && fabs(fill_etalep1)<2.5
             && fill_ngoodmus==0 && fill_ngoodeles==1 &&(fill_nloosemus+fill_nlooseeles)==1 && fabs(fill_Mla - 91.2) > 10;
   }

   cut_test2 = fill_photonhaspixelseed==0 && fill_photonet>25. && fill_jet1pt>40. && fabs(fill_jet1eta)<4.7
   ...
   cut_test3 = fill_Mjj >200 && fill_Mjj<500.;


            if (fill_Mjj <100) fill_Mjj = 120;
            if( cut_test1 && cut_test2 ){
               mjj_photonet->Fill(fill_Mjj, fill_photonet);
               mjj_photonet_w->Fill(fill_Mjj, fill_photonet, weight);
               mjj->Fill(fill_Mjj);
               mjj_w->Fill(fill_Mjj, weight);
