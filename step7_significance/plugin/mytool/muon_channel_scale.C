...
//cout<<"ok3.1.1"<<endl;
	...
	lep_ID_scale=ID->GetBinContent(ID->FindBin(ptlep_tmp,fabs(etalep)));
//cout<<"ok3.1.2"<<endl;

    if(sys_error == "up") lep_ID_scale = lep_ID_scale + ID->GetBinErrorUp(ID->FindBin(ptlep_tmp,fabs(etalep)));
//cout<<"ok3.1.3"<<endl;

    if(sys_error == "low") lep_ID_scale = lep_ID_scale - ID->GetBinErrorLow(ID->FindBin(ptlep_tmp,fabs(etalep)));	
	return lep_ID_scale;
}
...
double get_muon_iso(double etalep, double ptlep, TH2D* iso, TString sys_error){
	...
	lep_iso_scale=iso->GetBinContent(iso->FindBin(ptlep_tmp,fabs(etalep)));
    if(sys_error == "up") lep_iso_scale = lep_iso_scale + iso->GetBinErrorUp(iso->FindBin(ptlep_tmp,fabs(etalep)));
    if(sys_error == "low") lep_iso_scale = lep_iso_scale - iso->GetBinErrorLow(iso->FindBin(ptlep_tmp,fabs(etalep)));	
