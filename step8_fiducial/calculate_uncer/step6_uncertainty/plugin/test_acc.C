...
void test::creat_hist(){
   ...
   for(int j = 0; j<21; j++){
      ...
      h_in[j] = new TH1D("hist_in_"+uncertainty_hist_name[j],"hist_in_" +uncertainty_hist_name[j], 7, 0, 7);
      h_out[j] = new TH1D("hist_out_"+uncertainty_hist_name[j],"hist_out_" +uncertainty_hist_name[j], 7, 0, 7);

   }
    
   for(int j2 = 0; j2<4; j2++){
      h_jecr[j2] = new TH1D("hist_"+uncertainty_jecr[j2],"hist_"+uncertainty_jecr[j2], 7, 0, 7);   
      h_in_jecr[j2] = new TH1D("hist_in_"+uncertainty_jecr[j2],"hist_in_"+uncertainty_jecr[j2], 7, 0, 7);
      h_out_jecr[j2] = new TH1D("hist_out_"+uncertainty_jecr[j2],"hist_out_"+uncertainty_jecr[j2], 7, 0, 7);
...
//                  cout<<jentry<<" pt: "<<genjet1pt<< " "<<genjet2pt << " "<<fill_jet1pt<<" "<<fill_jet2pt<<endl;
                 ...
                 if (cut_gen) fill_hist(fill_Mjj, fill_deltaeta, h_in[j], fill_weight[j]);
                  if (!cut_gen) fill_hist(fill_Mjj, fill_deltaeta, h_out[j], fill_weight[j]);

               }
            }
            if (cut2) {
               fill_hist(fill_Mjj_JEC_up, fill_deltaeta_JEC_up, h_jecr[0], fill_weight_jecr[0]);
               if (cut_gen) fill_hist(fill_Mjj_JEC_up, fill_deltaeta_JEC_up, h_in_jecr[0], fill_weight_jecr[0]);
               if (!cut_gen) fill_hist(fill_Mjj_JEC_up, fill_deltaeta_JEC_up, h_out_jecr[0], fill_weight_jecr[0]);

            }
            if (cut3) {
               fill_hist(fill_Mjj_JEC_down, fill_deltaeta_JEC_down, h_jecr[1], fill_weight_jecr[1]);
               if (cut_gen) fill_hist(fill_Mjj_JEC_down, fill_deltaeta_JEC_down, h_in_jecr[1], fill_weight_jecr[1]);
               if (!cut_gen) fill_hist(fill_Mjj_JEC_down, fill_deltaeta_JEC_down, h_out_jecr[1], fill_weight_jecr[1]);
            }
            if (cut4) {
               fill_hist(fill_Mjj_JER_up, fill_deltaeta_JER_up, h_jecr[2], fill_weight_jecr[2]);
               if (cut_gen) fill_hist(fill_Mjj_JER_up, fill_deltaeta_JER_up, h_in_jecr[2], fill_weight_jecr[2]);
               if (!cut_gen) fill_hist(fill_Mjj_JER_up, fill_deltaeta_JER_up, h_out_jecr[2], fill_weight_jecr[2]);
            }
            if (cut5) {
               fill_hist(fill_Mjj_JER_down, fill_deltaeta_JER_down, h_jecr[3], fill_weight_jecr[3]);
               if (cut_gen) fill_hist(fill_Mjj_JER_down, fill_deltaeta_JER_down, h_in_jecr[3], fill_weight_jecr[3]);
               if (!cut_gen) fill_hist(fill_Mjj_JER_down, fill_deltaeta_JER_down, h_out_jecr[3], fill_weight_jecr[3]);
            }
