...
TH1D * hist = new TH1D("hist","hist",7,0,7);
...
void test::Loop(TDirectory * dir, TTree * tree)
{
   ...
   //if(m_channel == "muon") n = tree->GetEntries("lep == 13");
   //if(m_channel == "electron") n = tree->GetEntries("lep == 11");
...
*/
      ...
      if (fabs(fill_photonsceta) < pho_eta_high && fabs(fill_photonsceta) > pho_eta_low){
         if(m_type == "data") { 
           ...
           if(m_bORe == "barrel") weight = barrel_fake_photon_weight; //weight_jec_up = weight; weight_jec_down = weight; weight_jer_up = weight; weight_jer_down = weight;
           if(m_bORe == "endcap") weight = endcap_fake_photon_weight; //weight_jec_up = weight; weight_jec_down = weight; weight_jer_up = weight; weight_jer_down = weight;

         }
         else if(m_type == "doublefake") { 
           if(m_bORe == "barrel") weight = fake_lepton_weight * barrel_fake_photon_weight; //weight_jec_up = weight; weight_jec_down = weight; weight_jer_up = weight; weight_jer_down = weight;
           if(m_bORe == "endcap") weight = fake_lepton_weight * endcap_fake_photon_weight; //weight_jec_up = weight; weight_jec_down = weight; weight_jer_up = weight; weight_jer_down = weight;
...
//cout<<photon_ID_SF<<" "<<electron_ID_SF<<" "<<electron_Reco_SF<<" "<<electron_HLT_SF<<" "<<muon_ID_SF<<" "<<muon_iso_SF<<" "<<muon_HLT_SF<<endl;
