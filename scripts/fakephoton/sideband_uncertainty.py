...
for i in range(hist_sideband1_barrel.GetNbinsX()) :
    tmp_uncer_barrel = abs(hist_sideband1_barrel.GetBinContent(i+1) - hist_sideband2_barrel.GetBinContent(i+1))/(2*hist_sideband3_barrel.GetBinContent(i+1))
    tmp_uncer_endcap = abs(hist_sideband1_endcap.GetBinContent(i+1) - hist_sideband2_endcap.GetBinContent(i+1))/(2*hist_sideband3_endcap.GetBinContent(i+1))
...
fout = TFile( outputdir + '/sideband_uncertainty.root', 'RECREATE' )
fout.cd()
